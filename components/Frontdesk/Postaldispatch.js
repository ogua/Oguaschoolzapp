...
import Postaldispatchlist from '../../lists/postaldispatchlist';
...
function Postaldispatch () {
    ...
    const loaddata = () => {
        setLoading(true);
        axios.get(schoolzapi+'/postal-dispatch',
        {
        ...
        return Alert.alert(
            ...
            [
              ...
              },
              {
                ...
                onPress: () => {
                    setLoading(true);
                    axios.delete(schoolzapi+'/postal-dispatch/'+id,
                    {
                        headers: {Accept: 'application/json',
                        Authorization: "Bearer "+token
                    }
      ...
      const searchFilterFunction = (text) => {
          ...
          if (text) {
            ...
            const newData = data.filter(function (item) {
              const itemData = item.to || item.from
                ? (item.to.toUpperCase() ? item.to.toUpperCase(): item.from.toUpperCase())
                : ''.toUpperCase();
    ...
    return (
      ...
      <SafeAreaView style={{flexGrow: 1}}>
          ...
          <>
           {/* <View style={{marginVertical: 20}}>
                <View style={{flexDirection: 'row',justifyContent: 'flex-end', marginHorizontal: 20}}>
                    ...
                    <TouchableOpacity style={{flexDirection: 'row'}} onPress={()=> router.push('/admin/Frontdesk/create-edit-postal-dispatch')}>
            ...
            <Card>
                ...
                <FlatList
                    data={filterdata}
                    renderItem={({item})=> <Postaldispatchlist item={item} deletedata={deletedata} /> }
        ...
        <FAB
          icon="plus"
          style={styles.fab}
          onPress={()=> router.push('/admin/Frontdesk/create-edit-postal-dispatch')}
        />
      </SafeAreaView>
      </Provider>
    )
