...
import Totalattendancelist from '../../lists/Totalattendancelist';
...
function Totalattendance () {
    ...
    const fetchstudent = () => {
        ...
        setLoading(true);
        axios.get(schoolzapi+'/total-attendance/'+studentclass,
        {
            headers: {Accept: 'application/json',
            Authorization: "Bearer "+token
        }
        ...
        >
         ...
         <Text style={{fontSize: 15, fontWeight: 500}}>Student Class</Text>
               <DropDownPicker
                ...
                <FlatList
                    data={filterdata}
                    renderItem={({item})=> <Totalattendancelist item={item}  /> }
